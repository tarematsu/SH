-- Backfill canonical aliases once so normal queue resolution can use one
-- combined alias lookup instead of repeatedly probing metadata and three track
-- identity indexes.
INSERT OR IGNORE INTO sh_track_aliases(
  alias_type,alias_value,track_id,first_seen_at,last_seen_at
)
SELECT 'isrc',UPPER(TRIM(isrc)),id,first_seen_at,last_seen_at
FROM sh_tracks
...
SELECT 'spotify_id',TRIM(spotify_id),id,first_seen_at,last_seen_at
FROM sh_tracks
...
SELECT 'stationhead_track_id',CAST(stationhead_track_id AS TEXT),id,first_seen_at,last_seen_at
FROM sh_tracks
...
SELECT
  SUBSTR(canonical_key,1,INSTR(canonical_key,':')-1),
  SUBSTR(canonical_key,INSTR(canonical_key,':')+1),
  id,first_seen_at,last_seen_at
FROM sh_tracks
WHERE INSTR(canonical_key,':')>1
  AND SUBSTR(canonical_key,1,INSTR(canonical_key,':')-1) IN (
    'isrc','spotify_id','stationhead_track_id','legacy_track_id','legacy_name'
  );

-- Match the ready predicate before the priority ordering. The previous
-- priority-first index scanned not-yet-ready jobs before applying the due-time
-- bound.
...
-- New facts keep finalized playback position on the fact row. Historical rows
-- continue to resolve from the sparse context value without a backfill.
