...
BEGIN
  INSERT INTO sh_weekly_summary(
    ...
    NEW.week_of,
    CAST(strftime('%s',NEW.week_of||' 00:00:00') AS INTEGER)*1000,
    NEW.effective_at,1,1,NULL,NULL,NULL,
    ...
    CASE WHEN (SELECT stream_count FROM sh_email_stream_snapshots
      ...
      THEN NEW.stream_count-(SELECT stream_count FROM sh_email_stream_snapshots
        WHERE week_of=date(NEW.week_of,'-7 days') LIMIT 1) END,
    NULL,NULL,NULL,NULL,NULL,NULL,
    0.95,'["stationhead_email_recap","utc_period"]',
    unixepoch('now')*1000
  )
  ON CONFLICT(period_key) DO UPDATE SET
    ...
    quality_flags='["stationhead_email_recap","utc_period"]',
...
UPDATE sh_email_stream_snapshots SET validation_status=validation_status;
