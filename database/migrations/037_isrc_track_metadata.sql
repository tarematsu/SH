...
CREATE TRIGGER IF NOT EXISTS trg_sh_track_metadata_fill_isrc_insert
...
BEGIN
  ...
  SET isrc = (
    SELECT UPPER(TRIM(items.isrc))
    FROM sh_queue_items AS items
    WHERE items.spotify_id = NEW.spotify_id
      AND items.isrc IS NOT NULL
      AND TRIM(items.isrc) <> ''
    ORDER BY items.observed_at DESC
    LIMIT 1
  )
  WHERE spotify_id = NEW.spotify_id;
END;

CREATE TRIGGER IF NOT EXISTS trg_sh_track_metadata_fill_isrc_update
AFTER UPDATE OF spotify_id ON sh_track_metadata
WHEN NEW.isrc IS NULL OR TRIM(NEW.isrc) = ''
BEGIN
  UPDATE sh_track_metadata
  SET isrc = (
