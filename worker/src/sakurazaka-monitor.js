...
import { sakurazakaAuthStateId } from './sakurazaka-auth.js';

const API_BASE = 'https://production1.stationhead.com';
const COLLECTOR_ID = 'sh-sakurazaka46jp';
const SOURCE_PRIORITY = 100;

function cfg(env) {
  return {
    authStateId: sakurazakaAuthStateId(env),
...
async function session(env, config) {
  ...
  if (!row?.auth_token || !row?.device_uid) throw new Error('Stationhead Sakurazaka session unavailable');
  return row;
}
...
async function shouldProbeSolo(env, config, state, now = Date.now()) {
...
async function enrichTracks(env, config, queue, now) {
  const ingestFn = (activeEnv, type, data, observedAt) => internalIngest(
    savePrimaryIngest,
    activeEnv,
    type,
    data,
...
async function collectSoloDetails(env, config, auth, state, station, now) {
  ...
  if (queue) {
    ...
    if (nextHash !== state.lastQueueHash) {
      ...
      await enrichTracks(env, config, queue, now).catch((error) => {
        console.warn(JSON.stringify({ event: 'sakurazaka_track_metadata_failed', error: String(error?.message || error) }));
      });
  ...
  const history = await stationRequest(
    ...
    console.warn(JSON.stringify({ event: 'sakurazaka_comment_count_failed', error: String(error?.message || error) }));
    return null;
  });
...
async function probeSolo(env, config, auth, now, recoveredState = null) {
  ...
  if (!state.sessionId) {
    ...
    if (!opened.session_id) throw new Error('Sakurazaka session open returned no session ID');
...
export async function runSakurazakaMonitor(env, now = Date.now()) {
  if (!env?.MINUTE_DB || !env?.OTHER_DB) return { skipped: true, reason: 'database-binding-missing' };
  const config = cfg(env);
  try {
    const auth = await session(env, config);
    const soloState = await recoverSoloState(env, config);
    const probeDue = await shouldProbeSolo(env, config, soloState, now);
    if (probeDue) await probeSolo(env, config, auth, now, soloState);
    else if (soloState.lastError) await saveState(env, soloState.id, { ...soloState, lastError: null });
    return { skipped: !probeDue, reason: probeDue ? null : 'outside-official-window' };
  } catch (error) {
    const message = String(error?.message || error).slice(0, 1000);
    console.error(JSON.stringify({ event: 'sakurazaka_monitor_failed', error: message }));
    try {
