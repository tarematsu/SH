...
export async function readOptimizedHealth(env) {
  if (!env?.OTHER_DB || !env?.MINUTE_DB) return json({ ok: false, error: 'health bindings missing' }, 503);
  ...
  const [collector, latest] = await Promise.all([
    env.MINUTE_DB.prepare(`SELECT last_run_at,last_success_at,last_error_present,updated_at
      FROM sh_collector_read_model WHERE collector_id=? LIMIT 1`).bind(collectorId).first(),
    env.MINUTE_DB.prepare(`SELECT f.channel_id,c.station_id,f.observed_at
