...
export async function enrichIsrcTracks(env, queue, config = {}, dependencies = {}) {
  ...
  const limit = Math.max(0, Math.min(1, Math.floor(Number(config.isrcMetadataLimit ?? 1) || 0)));
  ...
  const stored = await db.prepare(`SELECT isrc,title,artist,fetched_at FROM sh_isrc_metadata
