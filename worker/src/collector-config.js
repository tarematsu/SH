...
export function configFromEnv(env) {
  return {
    ...
    appVersion: env.SH_APP_VERSION || '1.0.0',
...
export async function shJson(state, config, path) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      throw new Error(`SH API authentication failed (401). ${body.slice(0, 200)}`);
    }
    throw new Error(`SH API ${response.status}: ${path}${body ? ` | ${body.slice(0, 300)}` : ''}`);
  }

  try {
    return await response.json();
  } catch (error) {
    throw new Error(`SH API invalid JSON: ${path} | ${sanitizeFailureDetail(error?.message || error)}`);
  }
}
