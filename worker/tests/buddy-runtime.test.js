...
test('buddy runtime refreshes the guest session before collection even when stored auth is usable', async () => {
  ...
  const result = await collectBuddyPlaybackReady({ DB: db }, 3000, {
    ...
    acquireSession: async () => {
      acquireCalls += 1;
      return { authToken: 'fresh-token', deviceUid: 'fresh-device', tokenExpiresAt: 600000 };
    },
    collect: async (env) => {
  ...
  assert.equal(collectedAuth.authToken, 'fresh-token');
  assert.equal(collectedAuth.deviceUid, 'fresh-device');
});
