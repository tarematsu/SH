...
test('read-model hydration is deferred only for resolvable incomplete track presentation', () => {
  ...
  assert.equal(readModelNeedsHydration({
    queue: { value: { tracks: [{ title: 'Song', artist: null, thumbnail_url: 'image' }] } },
  }), false);
});
...
test('pending buddy playback stages are requeued as separate invocations', async () => {
  ...
  assert.equal(sent[0].body.direct_stage, 'parse-compute');
  assert.equal(sent[0].body.cycle_at, 1_800_000);
  assert.deepEqual(sent[0].options, { contentType: 'json', delaySeconds: 1 });
});

test('minute enrichment stages are delegated as separate Queue invocations', async () => {
  const sent = [];
  const env = {
  ...
  const result = await processMinuteEnrichment(env, {
    ...
    resolvePlayback: async () => ({ pending: true, stage: 'playback-patch' }),
  });

  assert.equal(result.pending, true);
  assert.equal(sent.length, 1);
  assert.equal(sent[0].body.stage, 'playback-patch');
  assert.deepEqual(sent[0].options, { contentType: 'json' });
});

test('minute rebuild stages remain separately retryable', async () => {
  const sent = [];
  const env = {
  ...
  const result = await processMinuteRebuildStage(env, {
    ...
    scan: async () => ({ pending: true, stage: 'rebuild' }),
  });

  assert.equal(result.pending, true);
  assert.equal(sent.length, 1);
  assert.equal(sent[0].body.stage, 'rebuild');
});
