import { CONSOLIDATED_SCRIPT, runWrangler } from './monitor-cutover-queues.mjs';
...
export async function deleteWorker(scriptName) {
  ...
  const absent = notFound(response, body);
  if ((!response.ok || body?.success === false) && !absent) {
...
export function deployConsolidatedWorker() {
  runWrangler(['deploy', '--config', 'wrangler.other.jsonc']);
}

export function rollbackConsolidatedWorker() {
  runWrangler(['rollback', '--name', CONSOLIDATED_SCRIPT,
