...
function deriveHostIngestUrl(ingestUrl) {
...
function normalizeComments(payload, stationId) {
  ...
  return items.map((chat) => ({
    comment_id: chat?.id ?? null,
    station_id: chat?.station_id ?? stationId ?? null,
    ...
    active_stream_days: chat?.active_stream_days ?? chat?.account?.active_stream_days ?? null,
