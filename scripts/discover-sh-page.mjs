...
function classifyUrl(url) {
  try {
    ...
    if (host.includes('production1.stationhead.com')) return 'sh-api';
    if (host.includes('stationhead.com')) return 'sh-web';
...
async function main() {
  const targetUrl = option('--url', process.env.SH_DISCOVERY_URL || DEFAULT_URL);
  const durationMs = Number(option('--duration-ms', process.env.SH_DISCOVERY_DURATION_MS || DEFAULT_DURATION_MS));
  ...
  page.on('response', (response) => {
    const task = (async () => {
      ...
      if (!['sh-api', 'sh-web'].includes(entry.source)) return;
  ...
  const pageState = await page.evaluate(() => ({
    ...
    buttons: [...document.querySelectorAll('button, [role="button"], a')].map((node) => ({
      tag: node.tagName,
  ...
  const summary = [
    ...
    '',
    '## Next step',
    'Open candidate-responses.json and the saved JSON bodies. The correct source should contain queue_tracks/tracks/current_station and track identifiers such as spotify_id.',
  ].join('\n');
