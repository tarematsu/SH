...
const outputDirectory = path.join(siteDirectory, 'public', '__sh_refactor_a9f6c3');
const ignoredDirectories = new Set(['.git', 'node_modules', '.wrangler', 'dist', 'build', 'coverage']);
...
writeFileSync(
  ...
  `${JSON.stringify({ pathHits, contentHits }, null, 2)}\n`,
  'utf8',
);
console.log(`SH refactor report: ${pathHits.length} paths, ${contentHits.length} files`);
