...
import {
  ...
  TRACK_HISTORY_SQL,
} from '../functions/lib/track-history-handler.js';
...
function createDatabase() {
  ...
  db.exec(`
    CREATE TABLE sh_queue_snapshots (
      ...
      queue_id INTEGER, start_time INTEGER, is_paused INTEGER, raw_json TEXT NOT NULL
    );
    CREATE TABLE sh_queue_items (
      id INTEGER PRIMARY KEY, observed_at INTEGER NOT NULL, station_id INTEGER NOT NULL,
      queue_id INTEGER, start_time INTEGER NOT NULL, position INTEGER NOT NULL,
      queue_track_id INTEGER, stationhead_track_id INTEGER, spotify_id TEXT,
      apple_music_id TEXT, deezer_id TEXT, isrc TEXT, duration_ms INTEGER,
      preview_url TEXT, bite_count INTEGER, raw_json TEXT NOT NULL,
    ...
    CREATE TABLE sh_track_metadata (
      ...
      thumbnail_url TEXT, spotify_url TEXT, source TEXT, fetched_at INTEGER, raw_json TEXT
    );
    CREATE TABLE sh_channel_snapshots (
      id INTEGER PRIMARY KEY, observed_at INTEGER NOT NULL, station_id INTEGER,
      is_launched INTEGER, is_broadcasting INTEGER
    );
  `);
  return db;
}
...
test('malformed legacy queue metadata does not erase otherwise valid history', () => {
  ...
  db.prepare(`INSERT INTO sh_queue_items (
    ...
    start, 1, 1, start, 0, 1, 'spotify-malformed', 180000, '{broken-json',
  );
