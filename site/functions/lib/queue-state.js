...
export function queueItemsFromRows(rows = [], state = null) {
  ...
  return rows.filter((row) => row.position != null).map((row) => ({
    ...
    spotify_id: row.spotify_id,deezer_id: row.deezer_id,isrc: row.isrc,
    duration_ms: row.duration_ms,preview_url: row.preview_url,bite_count: row.bite_count,
    title: row.title,artist: row.artist,display_title: row.display_title,
