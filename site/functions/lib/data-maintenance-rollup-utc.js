...
async function upsertSummary(db, table, key, aggregate, first, last, primaryHost, updatedAt) {
  ...
  await db.prepare(`INSERT INTO ${table}(
    ...
    '["live_rollup","utc_period"]', updatedAt,
  ).run();
  return true;
}

export async function rollupDailyUtc(db, period, now) {
  ...
  const streamValue = 'COALESCE(validated_stream_count,current_stream_count,total_listens)';
  const first = await db.prepare(`SELECT
      (SELECT ${streamValue} FROM sh_channel_snapshots
       WHERE observed_at>=? AND observed_at<? AND ${streamValue} IS NOT NULL
...
export async function rollupFromDailyUtc(db, table, range, now) {
  const aggregate = await db.prepare(`SELECT
