...
const LIKE_CHECKPOINT_MS = 60 * 60 * 1000;
...
export async function saveLeanSnapshot(db, observedAt, data) {
  ...
  const common = [
    ...
    stationId,
    bool(data?.is_launched),
    bool(data?.is_broadcasting),
    text(data?.chat_status),
    num(data?.listener_count),
    num(data?.online_member_count),
    num(data?.total_member_count),
    num(data?.guest_count),
    num(data?.total_listens),
    num(data?.stream_goal),
    num(data?.current_stream_count),
    num(data?.host_account_id),
    text(data?.host_handle),
    num(data?.broadcast_start_time),
  ];
  const velocityBinds = [stationId, observedAt - 120_000, observedAt];
  const compactRaw = rawJson(snapshotRawPayload(data));

  const updated = await db.prepare(`UPDATE sh_channel_snapshots SET
      observed_at=?,channel_id=?,channel_alias=?,channel_name=?,station_id=?,
      is_launched=?,is_broadcasting=?,chat_status=?,listener_count=?,
      online_member_count=?,total_member_count=?,guest_count=?,total_listens=?,
      stream_goal=?,current_stream_count=?,host_account_id=?,host_handle=?,
      broadcast_start_time=?,comment_velocity=(
    ...
    .bind(...common, ...velocityBinds, compactRaw, channelId, bucket, bucket + 60_000)
    .run();

  if (Number(updated?.meta?.changes || 0) > 0) return { inserted: false };

  await db.prepare(`INSERT INTO sh_channel_snapshots (
      observed_at,channel_id,channel_alias,channel_name,station_id,
...
async function loadQueueComparisonState(db, stationId, startTime, positions, trackKeys) {
...
function planLikeObservations(tracks, latestRows, observedAt) {
  ...
  return [...unique.entries()].filter(([trackKey, track]) => {
    const previous = latest.get(trackKey);
    return !previous
      || num(previous.like_count) !== num(track.bite_count)
      || observedAt - (num(previous.observed_at) ?? 0) >= LIKE_CHECKPOINT_MS;
...
export async function saveLeanQueue(db, observedAt, body) {
  ...
  const claim = await claimWrite(db, {
    dedupeKey: `station:${stationId ?? 0}:queue:${startTime ?? 0}:minute:${minuteBucket(observedAt)}:hash:${hash}`,
    dataType: 'queue',
    ...source,
    observedAt,
    hash,
  ...
  await db.prepare(`INSERT INTO sh_queue_snapshots (
    ...
    .bind(observedAt, stationId, queueId, startTime, bool(data?.is_paused), rawJson(payload))
    .run();
  ...
  const { existingRows, latestRows } = await loadQueueComparisonState(
    db, stationId, startTime, positions, trackKeys,
  );
