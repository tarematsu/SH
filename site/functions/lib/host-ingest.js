...
import { claimWrite, minuteBucket, payloadHash, sourceIdentity } from './ingest-claim.js';
...
async function openSession(db, observedAt, data) {
  ...
  const sourceScope = text(data.source_scope) || 'sakurazaka46jp_solo';
  ...
  return db.prepare(`SELECT id FROM sh_host_broadcast_sessions
    ...
    .bind(sourceScope, text(data.handle), num(data.station_id), startedAt)
    .first();
}
...
async function confirmSession(db, observedAt, data) {
  await db.prepare(`UPDATE sh_host_broadcast_sessions
    ...
    .bind(num(data.confirmed_at) ?? observedAt, observedAt, num(data.session_id))
    .run();
}
...
export async function saveStationSnapshot(db, observedAt, data) {
  ...
  const values = [
    sessionId,
    observedAt,
    text(data.source_scope),
    text(data.handle),
    num(data.account_id),
    num(data.station_id),
    num(data.broadcast_id),
    num(data.broadcast_start_time),
    bool(data.is_broadcasting),
    text(data.status),
    text(data.chat_status),
    num(data.listener_count),
    num(data.guest_count),
    num(data.total_listens),
    num(data.channel_id),
    text(data.channel_alias),
    num(data.current_track_id),
    text(data.current_spotify_id),
    num(data.queue_id),
    num(data.queue_start_time),
    num(data.comment_velocity),
  ];
...
async function saveQueue(db, observedAt, data) {
  ...
  await db.prepare(`UPDATE sh_host_broadcast_sessions SET track_count=? WHERE id=?`)
    .bind(num(count?.count) ?? 0, sessionId)
    .run();
}
...
export async function persistHostEvent(env, body) {
  const db = env?.OTHER_DB;
  const claimDb = env?.DB || db;
  if (!db) return { ok: false, status: 500, error: 'OTHER_DB binding missing' };
  if (!claimDb) return { ok: false, status: 500, error: 'claim DB binding missing' };
  ...
  if (type === 'solo_ws_event') {
    return { ok: false, status: 410, error: 'raw solo event ingestion retired' };
  }
  if (type === 'solo_comments') {
    const result = await saveSoloActivityCounts(db, observedAt, data);
    return { ok: true, type, accepted: result.accepted, total: result.total };
  }

  const source = sourceIdentity(body, {
    collectorId: body?.collector_id,
    collectorKind: 'worker',
  });
  const claim = await claimForType(claimDb, type, observedAt, data, source);
  ...
  switch (type) {
    case 'host_profile_snapshot':
      await updateSessionProfile(db, data);
      break;
    case 'solo_session_open': {
      const row = await openSession(db, observedAt, data);
      return { ok: true, type, accepted: true, session_id: row?.id ?? null };
    }
    case 'solo_session_confirm':
      await confirmSession(db, observedAt, data);
      break;
    case 'solo_station_snapshot':
      await saveStationSnapshot(db, observedAt, data);
      break;
    case 'solo_queue':
      await saveQueue(db, observedAt, data);
      break;
    case 'solo_session_close':
      await closeSession(db, observedAt, data);
      break;
    default:
      return { ok: false, status: 400, error: `unknown type: ${String(type || '')}` };
  }
  return { ok: true, type, accepted: true };
}

export async function hostIngestInternal({ request, env }) {
  ...
  try {
    const result = await persistHostEvent(env, parsed.body);
