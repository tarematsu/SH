...
function proxyStatement(statement, useCache) {
  return new Proxy(statement, {
    get(target, property) {
      if (useCache && property === 'first') return () => cachedPrediction(target);
...
function proxyDatabase(db) {
  return new Proxy(db, {
    get(target, property) {
      if (property === 'prepare') {
        return (sql) => proxyStatement(target.prepare(sql), normalize(sql) === targetSql);
      }
      const value = Reflect.get(target, property, target);
      return typeof value === 'function' ? value.bind(target) : value;
    },
...
export async function onRequestGet(context) {
  ...
  return legacyDashboard({ ...context, env: { ...context.env, DB: proxyDatabase(context.env.DB) } });
}
