...
export async function loadRankingRowsAndWeeks(statementOrDb, rankingStatement, weeksStatement) {
  if (rankingStatement && weeksStatement) {
    if (typeof statementOrDb?.batch === 'function') {
      return statementOrDb.batch([rankingStatement, weeksStatement]);
    }
    return Promise.all([rankingStatement.all(), weeksStatement.all()]);
  }

  const result = await statementOrDb.all();
  const rows = [];
  const weeks = [];
  const hostNames = [];
  const rankingTypes = [];
