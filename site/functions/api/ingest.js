...
async function saveSnapshot(db, observedAt, d) {
  ...
  const values = [
    observedAt, channelId, text(d.channel_alias), text(d.channel_name), num(d.station_id),
...
async function saveWsEvent(db, observedAt, data) {
  ...
  `).bind(
    ...
    text(data?.channel), text(data?.event), bucket, bucket + 5000, raw
  ).run();
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    if (type === 'snapshot') {
      ...
      claim = await claimWrite(env.DB, {
        dedupeKey: `station:${num(data.station_id) ?? 0}:queue:${num(data.start_time) ?? 0}:hash:${hash}`,
        dataType: type,
        ...source,
        observedAt,
        hash,
