...
async function loadSession(env = {}) {
  ...
  const authToken = normalizeBearer(row?.auth_token || env.SH_AUTH_TOKEN);
  const deviceUid = String(row?.device_uid || env.SH_DEVICE_UID || '').trim();
  if (!authToken || !deviceUid) {
    throw new Error('SH authenticated session is unavailable. Sync the collector session first.');
  }
...
export async function onRequestGet({ env = {} } = {}) {
  ...
  const requestHeaders = {
    ...
    'App-Version': env.SH_APP_VERSION || APP_VERSION,
  ...
  try {
    ...
    return jsonResponse({
      ...
      error: {
        ...
        message: timedOut ? `SH API did not respond within ${TIMEOUT_MS} ms.` : String(error?.message || error),
      },
    }, timedOut ? 504 : 502);
  } finally {
