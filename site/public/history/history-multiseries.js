(() => {
  ...
  function sampledGapThreshold(dates) {
    ...
    const gaps = times
      .slice(1)
      .map((time, index) => (time != null && times[index] != null ? time - times[index] : null))
  ...
  function drawSummary(rows, selectedIndex = null) {
    ...
    const active = METRICS.filter(
      ([key]) => sampled.some((row) => finiteNumber(row[key]) != null),
    );
    ...
    if (selected != null) {
      ...
      $('#chartDetail').innerHTML = `<time>${escapeHtml(formatDate(dates[selected]))}</time>`
        + `<div class="chart-detail-values">${series.map((item) =>
          `<div style="opacity:${item.alpha}"><i style="background:${item.color}"></i>`
          + `<strong>${escapeHtml(item.label)}</strong><span>${fmt(row[item.key])}</span></div>`,
        ).join('')}</div>`;
    }
  }
  ...
  setMode = function (mode) {
    ...
    if (['daily', 'weekly', 'monthly'].includes(mode)) {
      ...
      $('#chartTitle').textContent = '主要指標の推移';
      $('#chartFoot').textContent = '平均同接と再生数増加を強調表示。各指標は個別スケールです。';
    }
  };
