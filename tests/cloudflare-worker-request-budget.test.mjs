...
test('live queues remain counted while historical reconstruction is temporarily excluded', () => {
  ...
  assert.equal(QUEUE_MESSAGES_PER_DAY['stationhead-pages-read-model-publication'], 2_880);
...
test('the two-Worker topology stays below 80000 counted requests per day', async () => {
  ...
  assert.equal(report.queue_consumer_requests, 36_384);
  assert.equal(report.scheduled_requests, 1_728);
